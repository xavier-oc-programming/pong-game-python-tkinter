...
def main() -> None:
    ...
    while True:
        ...
        mode = display.show_welcome()   # "auto" | "manual" | None (Q = quit)
        if mode is None:
            display.close()             # sys.exit(0) — only true quit path

        # Restore score strip that show_welcome() cleared
        display.render_score(tracker.score)
        display.screen.update()
        ...
        # --------------------------------------------------------------
        # Game loop — break → outer loop → title screen
        # --------------------------------------------------------------
        while True:
            ...
            if point_scored:
                if tracker.is_game_over():
                    ...
                    keys["pause"] = False   # discard Space pressed during delay
